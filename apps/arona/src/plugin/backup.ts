 ...
 * @returns
 */
export function createBackupPlugin({ groupId }: { groupId: number }): EventPlugin {
  return {
    onMessage: (event) =>
      Effect.gen(function* () {
        const db = yield* DbService;
        if (event.message_type !== "group" || event.group_id !== groupId) {
          return;
        }
        ...
        for (const segment of event.message) {
          ...
          if (segment.type === "text") {
            yield* processTextMessage(groupId, event, segment.data.text, segmentIndex).pipe(
              Effect.catchAll((e) =>
                Effect.sync(() => logger.error({ msg: "Failed to process text", error: e.message }))
              )
            );
          } else if (segment.type === "image") {
            yield* processImageMessage(groupId, event, segment.data.url, segmentIndex).pipe(
              Effect.catchAll((e) =>
                Effect.sync(() => logger.error({ msg: "Failed to process image", error: e.message }))
              )
            );
          }
...
function processTextMessage(groupId: number, event: OneBotMessageEvent, text: string, segmentIndex: number) {
...
function processImageMessage(groupId: number, event: OneBotMessageEvent, imageUrl: string, segmentIndex: number) {
  return Effect.gen(function* () {
